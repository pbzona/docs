...
published: 'Wednesday, March 28th, 2017'
modified: 'Wednesday, March 28th, 2017'
modified_by:
  name: Linode
title: 'Advanced SSH Server Security'
contributor:
  name: Damaso Sanoja
...
If you've got a Linode up and running, there's a good chance you've been using SSH (Secure Shell) to access it from your computer. Although SSH is a secure protocol, most system compromises are a result of human error or failure to take advantage of the security features it offers. In this guide, we'll cover a few key features provided by OpenSSH.

[OpenSSH](http://www.openssh.com/) is a suite of connectivity tools that sysadmins use daily to access remote servers. From a security point of view it's the "front door" for remote logins, and that's why is extremely important to harden it as much as possible. The aim of this guide is to build upon our [Securing Your Server](/docs/security/securing-your-server/) guide by means of easy steps that can be implemented in just a few minutes.

**Assumptions:**

* You'll be deploying a server open to the internet 24/7.
* Your primary concern is security over simplicity or convenience. Be aware that more security implies, in most cases, less convenience.
...
{: .caution}
> Before running these commands on a production server, be aware that depending on the size of the keys you're generating, this may take anywhere from a minute to several hours and use significant CPU power.
...
`DenyUsers phpadmin tempuser dbuser@20.20.20.20` - The users `phpadmin` and `tempuser` have no SSH access at all, but the user `dbuser` is only blocked if is connecting from the specific IP address 20.20.20.20

### Selectively Allow Users

`AllowUsers superadmin megauser@yourdomain.tld dbadmin@10.10.10.10` - only the users `superadmin`, `megauser` and `dbadmin` have SSH access, but `megauser` and `dbadmin` can only connect from specific domains/IP addresses. On the other hand, `superadmin` has the ability to connect from any computer.
...
This kind of password is very hard to crack, but has the obvious disadvantage of being inconvenient each time you log to your server. The recommended method is to use a good password manager, because using a plain-text file for this kills the purpose of using the password in the first place. 

To add the generated password to your existing private key:

		ssh-keygen -p -f ~/.ssh/id_rsa

This assumes you keep your client SSH key in their default location `~/.ssh/id_rsa`. You can modify the file location as needed, and use the same command to change your password in the future.
...
However, this strategy involves a time-consuming process to configure the "jailed" user. For the purposes of this guide we'll setup a bare enviroment with some basic commands available for the `restricted-user`.
...
5.  Install the `bash` shell in the jailed directory:

        sudo cp -v /bin/bash /home/chroot/restricted-user/bin

6.  For each binary copied into the jailed directory, we need to determine which shared libraries are needed. This is done through the `ldd` command:

        sudo ldd /bin/bash

7.  Manually copy each library from the output to the corresponding location in our `restricted-user` directory. This is necessary because our user won't have access ourside its own jail. 
...
9.  Copy user information to the jailed directory:
...
Keep in mind that our user can't use any command or binary that is not manually installed in its jailed environment. The complete process to setup a user for specific tasks it overwhelming but has the advantage that is protected in the event that user is compromised.
...
According to that if you decide to limit an idle SSH connection to 2 minutes before disconnecting the client you can configure it as follows:
...
This step will not harden your server security, but your legal-conscientious-parameters. In some locations, the simple fact of warning unauthorized users of the consequences of their actions is determinant for taking legal action. To display a warning banner each time a user log to your server add the following line to the `/etc/ssh/sshd_config` configuration:
...
The value `/WarningMessage/Location` is the path to your banner text file. The following banner example was taken from OpenSSH Ubuntu help:
